...
public:
    ...
    std::vector<K> getAllKey() const; // 获取所有键

    std::vector<V> getAllData() const; // 获取所有值
};
...
template <typename K, typename V>
std::vector<K> AVLTree<K, V>::getAllKey() const
{
    std::vector<K> keys;
...
template <typename K, typename V>
std::vector<V> AVLTree<K, V>::getAllData() const
{
    std::vector<V> data;
    std::vector<Node*> stack;
    Node* node = root;
